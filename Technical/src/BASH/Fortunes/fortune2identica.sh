...
{
	curl -u USUARIO:CONTRASEÑA -d status="$(cat $BASE)" http://identi.ca/api/statuses/update.xml > /dev/null
	notify-send "Fortune posteado en Identi.ca" "$(cat $BASE)
Gracias por usar el script http://angelverde.info"
}
else
	notify-send "Fortune To Identi.ca Error" "Caracteres Máximos no soportados :) , notificar a alvaradoangel57@gmail.com o http://angelverde.info por favor"
fi
